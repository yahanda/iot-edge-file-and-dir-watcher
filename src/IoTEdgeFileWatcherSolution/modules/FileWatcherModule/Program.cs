namespace FileWatcherModule
{
    ...
    class Program
    {
        ...
        private const int DefaultRetentionTime = 60000;
         ...
         private static async void ThreadBody(object userContext)
        {
            ...
            while (true)
            {
                ...
                {                    
                    ...
                    var files = Directory.GetFileSystemEntries("exchange", SearchPattern);

                    //System.Console.WriteLine($"{DateTime.UtcNow} - Seen {files.Length} files with pattern '{SearchPattern}'" );
                    System.Console.WriteLine($"{DateTime.UtcNow} - Seen {files.Length} directory objects with pattern '{SearchPattern}'" );
                    ...
                    if (files.Length > 0)
                    {
                        foreach(var fileName in files)
                        {
                            ...
                            var isDirectory = File.GetAttributes(fi.FullName).HasFlag(FileAttributes.Directory);

                            if (isDirectory)
                            {                                
                                System.Console.WriteLine($"{DateTime.UtcNow} - Directory found: '{fi.FullName}'");
                                ...
                                // Sleep until file has been fully written
                                System.Console.WriteLine($"{DateTime.UtcNow} - Sleep for {RetentionTime} msec" );
                                Thread.Sleep(RetentionTime);

                                // Send the message
                                await client.SendEventAsync("output1", message);
                                System.Console.WriteLine($"{DateTime.UtcNow} - Sending message: {messageBody}");

                                var targetFullFilename = fi.FullName + RenameExtension;
                                
                                Directory.Move(fi.FullName, targetFullFilename);
                                System.Console.WriteLine($"{DateTime.UtcNow} - Renamed '{fi.FullName}' to '{targetFullFilename}'");                            
                            ...
                            else
                            {
                                ...
                                System.Console.WriteLine($"{DateTime.UtcNow} - File found: '{fi.FullName}' - Size: {fi.Length} bytes.");
        ...
        public static int RetentionTime {get; set;} = DefaultRetentionTime;
        ...
        static async Task OnDesiredPropertiesUpdate(TwinCollection desiredProperties, object userContext)
        {
            ...
            try
            {
                ...
                if (desiredProperties.Contains("retentionTime")) 
                {
                    if (desiredProperties["retentionTime"] != null)
                    {
                        RetentionTime = desiredProperties["retentionTime"];
                    }
                    else
                    {
                    ...
                    Console.WriteLine($"RetentionTime changed to '{RetentionTime}'");

                    reportedProperties["retentionTime"] = RetentionTime;
                } 
                else
                {
                    Console.WriteLine($"RetentionTime ignored");
                }
